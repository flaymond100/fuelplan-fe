...
const FUEL_FORM_LABELS: Record<FuelForm, string> = {
  gels: 'Gels',
  chews: 'Chews',
  bars: 'Bars',
  drink_mix: 'Drink mix',
...
export default function Profile() {
  ...
  async function handleSubmit(e: SyntheticEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      runningThresholdSecPerKm: parsePace(form.runningThresholdText),
      maxHr: form.maxHr ? Number(form.maxHr) : null,
      weeklyTrainingHours: form.weeklyTrainingHours ? Number(form.weeklyTrainingHours) : null,
      sweatRate: form.sweatRate || null,
      maxCarbsGHr: form.maxCarbsGHr ? Number(form.maxCarbsGHr) : null,
      caffeineTolerance: form.caffeineTolerance || null,
      fuelForms: Array.from(form.fuelForms),
      diet: form.diet || null,
      restrictions: Array.from(form.restrictions),
      restrictionsOther: form.restrictionsOther || null,
      avoidNotes: form.avoidNotes || null,
    ...
    try {
      ...
      const msg = message.startsWith('404 ')
        ? "Backend isn't live yet — PUT /api/profile pending."
  ...
  if (loadState === 'loading') {
    return (
      <div>
        <PageHeader title="Profile" subtitle="Body, training, fuelling, and dietary preferences." />
